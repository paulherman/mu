...
bool file_buffer_read(struct file_buffer *fb, const char *path) {
  ...
  do {
    size_t read_length = fread(buffer, sizeof(uint8_t), 4096, file);
    if (read_length > 0) {
      length += read_length;
      num_batches++;
      contents = realloc(contents, (4096 * num_batches) * sizeof(uint8_t));
      memcpy(contents + 4096 * (num_batches - 1), buffer, sizeof(uint8_t) * read_length);
